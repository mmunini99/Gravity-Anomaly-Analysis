...
**Why gravity and isostasy modeling matters here:** Bouguer anomaly and residual gravity maps (as computed in `Script3`, `Script6`, and `Script_10`) can help characterize subsurface density contrasts associated with the clay-rich lacustrine sediments versus the surrounding volcanic rock — the same contrast responsible for the differential subsidence Carreón-Freyre and other researchers have documented in areas like Iztapalapa. Isostatic Moho modeling (`Script8`, `Script9`) and horizontal gradient mapping (`Script11`) provide complementary context on deeper crustal loading and the location of density-contrast boundaries (e.g., basin edges), which can align with zones of maximum differential ground motion.
...
They are numbered (`Script1`, `Script2`, ... `Script15`) and are meant to be run **in order**, since several later scripts consume grids produced by earlier onScript.
...
## Script summariScript

| # | Script | Purpose |
|---|---|---|
| 1 | `Script1_GravityAnomalyMap.bat` | Plots the free-air / observed gravity anomaly grid as a color raster with coastline, contour linScript (interval 30 mGal), and a color scale. |
| 2 | `Script2_GravityAnomaly_ContourMap.bat` | Same gravity anomaly data, shown as a contour-only map over a gray landmass (no raster fill) — useful for a cleaner structural view. |
| 3 | `Script3_SimpleBouguerAnomalyMap.bat` | Maps the simple Bouguer anomaly (gravity anomaly corrected for the attraction of topographic mass using a standard reduction density), with contours at 30 mGal. |
| 4 | `Script4_GeoidMap.bat` | Maps the geoid height grid — the equipotential surface reprScriptenting mean sea level extended under the continents. |
| 5 | `Script5_TopographyMap.bat` | Plots the ETOPO1 topography/bathymetry grid using the `etopo1` color palette, contoured every 500 m. |
| 6 | `Script6_CompleteBouguerCalculation.bat` | Two-part script: (a) maps the gravity disturbance due to the topographic field, then (b) computScript the **complete Bouguer anomaly** by subtracting that topographic gravity effect from the observed gravity anomaly, producing `BG_complete_COM.grd`. This is the key derived grid feeding several later scripts. |
| 7 | `Script7_isostaticMohoCalculation.bat` | Uses `grdflexure` to model the flexural (Airy-type) isostatic Moho, assuming an elastic thickness of 10 km, from the topography load. |
| 8 | `Script8_isostaticMohoCalculationT50.bat` | Same isostatic Moho calculation as script 8, but with a stiffer elastic thickness of 50 km, for comparison of lithospheric rigidity assumptions. |
| 9 | `Script9_Horizontal_grad.bat` | Computes the horizontal gradient magnitude of the complete Bouguer anomaly (√(∂x² + ∂y²)) — commonly used to highlight the edges of density contrasts, e.g. basin margins or fault zones. |
| 10 | `Script_10_BG_residual.bat` | Splits the complete Bouguer anomaly into **regional** (long-wavelength, `grdfilter -Fg50`) and **residual** (short-wavelength) components, isolating shallow/local density anomalies from deep crustal structure. |
| 11 | `Script11_LowFilter.bat` | Applies a low-pass Gaussian filter (`-Fg30`) to the complete Bouguer anomaly to isolate the regional gravity signal. |
| 12 | `Script12_Hill.bat` | Generates an illuminated hillshade relief of the topography grid (azimuth 315°) for cartographic terrain shading. |
